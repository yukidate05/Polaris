...
export default function NotionCallbackScreen() {
  ...
  useEffect(() => {
    ...
    notionService.exchangeCode(code)
      .then(({ workspaceName }) => {
        setLabel(`「${workspaceName}」と連携しました`);
        setState('success');
        ...
        {state === 'loading' && (
          <>
            <View style={styles.iconRing}>
              <Ionicons name="document-text-outline" size={28} color={Colors.brand.primary} />
            </View>
            <Text style={styles.cardTitle}>Notionと連携中...</Text>
