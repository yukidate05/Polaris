...
export default function ChatworkCallbackScreen() {
  ...
  useEffect(() => {
    ...
    chatworkService.exchangeCode(code)
      .then(({ name }) => {
        setLabel(`「${name}」と連携しました`);
        setState('success');
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.card}>
        <LinearGradient
          colors={['rgba(229,57,53,0.12)', 'rgba(107,140,255,0.08)']}
        ...
        {state === 'loading' && (
          <>
            <View style={styles.iconRing}>
              <Ionicons name="chatbubbles-outline" size={28} color="#E53935" />
            </View>
            <Text style={styles.cardTitle}>Chatworkと連携中...</Text>
...
const styles = StyleSheet.create({
  ...
  tint: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(10,15,35,0.55)' },
  iconRing: {
    width: 64, height: 64, borderRadius: 32,
    backgroundColor: 'rgba(229,57,53,0.12)',
    borderWidth: 1, borderColor: 'rgba(229,57,53,0.28)',
  ...
  cardTitle: { fontSize: 18, fontWeight: '700', color: '#fff', letterSpacing: -0.2 },
  cardSub:   { fontSize: 14, color: 'rgba(255,255,255,0.55)', textAlign: 'center', lineHeight: 20 },
});
