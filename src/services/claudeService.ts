...
━━ Notion（最近更新されたページ） ━━
${params.notionMyName ? `【重要】Notionでの${params.userName}さん自身のアカウント名は「${params.notionMyName}」です。「${params.notionMyName}」が更新者として表示されるページは${params.userName}さん本人の更新であり、第三者の行動として報告しないこと。\n` : ''}${params.notionPages.length > 0
...
${params.chatworkMyName ? `【重要】Chatworkでの${params.userName}さん自身のアカウント名は「${params.chatworkMyName}」です。「${params.chatworkMyName}」が送信者のメッセージは${params.userName}さん本人の発言であり、第三者の行動として報告しないこと。` : ''}
...
【3つのセクション構成（全体で約${params.targetMinutes ?? 5}分・約${Math.round((params.targetMinutes ?? 5) * 300)}字）】
...
制約:
- 各セリフは必ず80字以上120字以下（厳守）
- 各chapterは5セリフの対話（全体で約${Math.round((params.targetMinutes ?? 5) * 300)}字・約${params.targetMinutes ?? 5}分）
