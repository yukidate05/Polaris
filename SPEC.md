...
最終更新: 2026-07-07（生成フローのサーバー側移行が完了。vc38内部テスト配信済み・実機で4件の不具合解消を確認（日本語/英語とも）・`master`へマージ済み）
...
## ブリーフィング生成フロー（vc38〜: サーバー側ワーカー方式）

クライアント（`home.tsx`）はジョブを投入して`onSnapshot`購読するだけ。スクリプト生成・ニュース生成・TTSはすべて`briefingWorker`（Firestoreトリガー）がサーバー側で実行するため、**Androidのプロセスキルでも生成は中断せず、再起動時は続きから復元される**。

```
[クライアント: home.tsx]
1. status: 'fetching'
   googleDataService.fetchAll(accessToken)  ← Expo Go/トークンなしは MOCK_GOOGLE_DATA（従来のクライアント内生成にフォールバック）
   sessionService.get(uid) / アクセス制御チェック（checkIsPro + checkAccess → PaywallModal）

2. status: 'generating_script'
   prepareBriefingInputs()                  # 記憶・外部ツールデータ収集（briefingService）
   claudeService.buildBriefingPrompt()      # プロンプトはクライアントで構築
   submitBriefingJob(uid, ...)              # users/{uid}/briefingJobs/current に status:'queued' で書き込み
   subscribeBriefingJob(uid, ...)           # onSnapshotでジョブを購読、結果をstoreへ反映

[サーバー: briefingWorker（Firestoreトリガー・ステートマシン、各ステージをトランザクションでクレーム）]
Stage A (queued→):        台本生成（Gemini）→ script書き込み（クライアントは即デバイスTTS再生可能）
                          → lastFreeUseAt記録 → 【Proのみ】ニュース2段階生成（キャッシュ確認→テーマ選定→深堀り）
Stage B (tts_pending→):   本編TTS（Gemini multiSpeaker、失敗時Google TTSフォールバック）→ Storage保存
                          → audioUrl/audioEngine/audioDurationSec書き込み → FCM完了プッシュ送信（9言語）
Stage C (news_tts_pending→): ニュースTTS → newsAudioUrl書き込み → status:'completed'

[クライアント: 復元]
3. 起動時ハイドレーション: 進行中（20分以内）or 完成直後（60分以内）のジョブがあれば
   自動生成せずジョブを購読して復元（プロセスキル後の二重生成・二重課金を防止）

[player.tsx]
   PlayPhase: 'briefing' → 'transition'（遷移セリフ）→ 'bridge'（BGM最低10秒）→ 'news'
```

- 通知: `getDevicePushTokenAsync`で取得したFCMトークンを`users/{uid}.fcmToken`に保存し、ワーカーが`admin.messaging().send()`で本編音声完成時に送信（Expo push service不使用・EAS credential設定不要）
- ジョブは`jobId`で世代管理。再生成でジョブが上書きされても旧ワーカーインスタンスは`guardedUpdate`（jobId照合トランザクション）により新ジョブを壊せない
- ニュースの実音声長はWAVヘッダーから算出して`newsAudioDurationSec`に記録（テキスト推定の約0.6倍が実測値。行数計算はこの実測から補正済み）
...
**⚠️ vc38以降、この処理は`briefingWorker`（`functions/src/briefingWorker.ts`）内でサーバー側実行される。** クライアント側の`claudeService.selectNewsTheme()`/`generateNewsCast()`/`briefingService.generateNewsSegment()`は現在メインフローから呼ばれない休眠コード（プロンプト内容はサーバー版と同一の英語指示文ベース）。以下のロジック説明はサーバー版にも当てはまる。

`selectNewsTheme`（Phase1: テーマ選定）→`generateNewsCast`（Phase2: 深堀り生成）の順で呼ぶ。どちらもGoogle Search grounding（`useSearch:true`）付きのGemini呼び出し。
...
- ⚠️ **2026-07-09: GoogleからADA-CASA AL1（旧Tier 2）評価の正式要求が届いた。期限: 2026-10-06。** 制限付きスコープ（`gmail.readonly`）の検証続行条件で、以後毎年更新が必要。評価には2〜6週間かかるため早期着手推奨（遅くとも2026年9月上旬までに開始、8月中の見積もり依頼が安全）。AL2（旧Tier 3、インフラ含む包括評価・Marketplaceバッジ用）は不要でAL1で足りる。メール文面上、認定ラボへの依頼が前提のため無料セルフスキャン案は使えない見込み
- **料金（2026-07-09にTAC Security公式で確認）**: Basic **$675**（評価1回+再検証2サイクル+修正サポート+LoV、1〜3週間）/ Premium $855（再検証無制限）/ Enterprise $3,600。他ラボは$3,000超もあるためTAC Basicが第一候補。旧記載の「$75〜150」は古い情報で誤り。**毎年発生する固定費**として収益計画に織り込むこと（月換算約8,500円）
- 次のアクション: 8月中にTAC SecurityへAL1 Basicの申し込み→ASVSベース質問票+スキャン提出（質問票準備の参考事例: https://meetorbis.com/blog/how-we-passed-google-casa-tier-2-with-claude ）
...
4. Play Consoleにアップロード（`upload_aab.mjs`はプロジェクトルート配置済み）
   ...
   ```
   前提: サービスアカウントJSON `polaris-app-yukid-458b1ff906c2.json` がプロジェクトルートに存在すること（`upload_aab.mjs`と同階層、`path.join(import.meta.dirname, ...)`で解決。gitignore対象・マシンごとに手動配置が必要）。アップロード先: `internal`トラック。
...
```
端末側での反映確認は、内部テスト参加リンクからインストールしたPlay Storeアプリ内でのみ可能（一般公開ページでは不可）。反映まで数分〜数十分のタイムラグがあることがある。
...
## 生成フローのサーバー側移行（2026-07-07完了・実機検証済み・masterへマージ済み）

`feature/resilient-generation`ブランチで実装し、実機検証後に`master`へfast-forwardマージ（`036a14e`）。masterはvc19相当で長期間止まっていたが、このマージでvc20〜38の全作業が反映された。`feature/two-phase-news`・`feature/resilient-generation`はロールバック用にリモートに残置。

vc37実機テストで報告された4件の不具合調査の結果、①生成中にプロセスキルされると最初から再生成（API課金二重発生）②完了通知が来ない ④ホストがAria/Crestにならない、の3件は「生成パイプラインがクライアントJSの生存に依存している」という同一の根本原因と判明（Androidの省電力によるプロセスキル→状態全損→再生成→geminiTtsレート制限に衝突→Google TTSへサイレントフォールバック=④）。応急処置ではなくサーバー側オーケストレーションへ移行する。

**設計方針**:
- ジョブドキュメント `users/{uid}/briefingJobs/current` を生成状態の正本にする。クライアントはジョブを投入して`onSnapshot`で購読するだけ（プロセスが死んでも購読再開すれば続きから表示）
- Firestoreトリガーのワーカー関数がステートマシンとして動く: `queued`→(Stage A: 台本生成+ニューステキスト生成)→`tts_pending`→(Stage B: 本編TTS+プッシュ通知送信)→`news_tts_pending`→(Stage C: ニュースTTS)→`completed`。各ステージ540秒制限内・ステータス遷移をトランザクションで排他
- 台本プロンプトはクライアントが構築してジョブに同梱（記憶・セッション・Google/外部ツールデータはクライアント側にあるため）。ニュース2フェーズのプロンプト構築はサーバーへ移植（Phase2がPhase1の結果に依存するため）
- 通知はExpo push serviceを使わずFCM直（`getDevicePushTokenAsync`→`users/{uid}.fcmToken`→ワーカーが`admin.messaging().send()`、9言語対応）。EAS側のFCM credential設定が不要
- TTSはワーカーが直接実行（HTTPエンドポイント経由でないためレート制限に衝突しない=④解消）。Gemini TTS失敗時のGoogle TTSフォールバックもサーバー側で行い、使用エンジンをジョブdocに記録
...
- [x] vc38ビルド・アップロード→実機で確認

**実機検証結果（2026-07-07、vc38）**: ①バックグラウンド移動→復帰で再生成されない ②完了プッシュ通知が届く ③ニュース実測6分 ④ホストAria/Crest音声 — 報告された4件すべて解消をユーザー実機で確認済み。英語設定でも音声生成に問題ないことを確認済み。

**復元仕様（起動時ハイドレーション）**: アプリ起動時、完成ジョブが60分以内なら再生成せず復元（内容は同一）。進行中ジョブ（20分以内）は購読を再開して続きから表示。それより古い場合のみ新規生成。データは削除されず次の生成で上書き（Storage容量は1ユーザーあたり常に最新1世代分・約30MBで一定）。
...
- [ ] Google OAuth検証（データアクセス審査中）→ 通過待ち
- [ ] **CASA AL1評価 — 段階戦略を採用（2026-07-09決定）**: 今は払わず、未検証のまま（警告+100人制限、アプリ動作に支障なし）テスター12人→Alpha通過→一般公開し、100人枠内で課金意欲を検証。**2026年8月末に支払い判断**（シグナルあり→9月頭にTAC Basic $675申込→10/6期限に間に合う。シグナルなし→撤退/転換）。回収ライン=Pro 9人/月。代替案（スコープ削減・端末内完結・Nylas/Unipile共有OAuth・Gmail削除）は全て検討の上却下（詳細は収益化メモリ）。注意: OAuth同意画面が「テスト」ステータスだとリフレッシュトークン7日失効の仕様があるため、毎週再ログイン現象が出たら公開ステータスを確認すること
...
- [ ] EAS build creditsは請求期間分を**使い切り済み**（2026-07-07 vc38ビルド時点で100%到達）。以降のビルドはpay-as-you-go課金: Android medium（本プロジェクトのデフォルト）は**$1/ビルド**（large $2、iOS medium $2/large $4）。次の請求サイクルまでビルド1回≒150円の実費前提で計画すること。詳細: https://expo.dev/accounts/semla/settings/billing
...
### 2026-07-07: 生成フローのサーバー側移行（vc38）+ masterマージ

vc37実機テストで報告された4件の不具合（①BG復帰で再生成 ②完了通知が来ない ③ニュース3分止まり ④ホスト音声が違う）を調査し、①②④が同一根本原因（生成パイプラインのクライアントJS依存）と特定してサーバー側ワーカー方式へ移行。詳細は「生成フローのサーバー側移行」セクション参照。1日で実装→本番E2E検証→vc38ビルド・配信→実機で4件全て解消確認→masterマージまで完了。

- 検証時の学び: 実TTS音声はテキスト推定（5字/秒）の約0.6倍しかない（実測9.2秒/行）。行数バジェットは必ず実音声長ベースで補正すること
- 副次修正: `player.tsx`が参照する未定義i18nキー9件（`player_empty_msg`＋a11y系8件、プレイヤー空状態に生キー名が表示されていた）を9言語分追加
- 運用メモ: 新マシンでは`functions/lib`が未生成のため`firebase deploy`前に`cd functions && npm run build`が必要。Firestoreトリガー初回デプロイはEventarc伝播待ちで1回失敗→リトライで成功する
